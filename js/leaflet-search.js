...
(function (factory) {
  ...
  L.Control.Search = L.Control.extend({
      ...
      if (this.options.tooltipLimit) {
        ...
        else
        {
          ...
          for (let index = 0; index < records.length; index++) {
            ...
            this._tooltip.appendChild(this._createTip(inputValue, records[index]))
            this._countertips++
          }
        }
      }
    ...
    _searchInLayer: function (layer, propName, baseProp = 'options') {
      ...
      if (layer instanceof L.Marker || layer instanceof L.CircleMarker) {
        ...
        if (self._getPath(layer.options, propName)) {
          ...
          loc = /*layer.getBounds().getCenter()*/ L.latLngBounds(layer.feature.geometry.coordinates);
          // loc.layer = layer
          retRecords[propName] = layer.feature.properties[propName];
          retRecords[this._input.value] = loc;
          retRecords['layer'] = layer;
        } else {
        ...
        layer.eachLayer(function (layer) {
          arr_records.push(self._searchInLayer(layer, propName))
        })
        return arr_records; 
      }
      return retRecords;
    ...
    _recordsFromLayer: function () { // return table: key,value from layer
      ...
      this._layer.eachLayer(function (layer) {
        retRecords.push(self._searchInLayer(layer, propName));
      })
    ...
    showLocation: function (latlng, title) { // set location on map from _recordsCache
      ...
      self._moveToLocation(latlng, title, self._input.value, self._map)
